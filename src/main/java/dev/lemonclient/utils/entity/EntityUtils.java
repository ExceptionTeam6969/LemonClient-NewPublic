...
public class EntityUtils {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((IEntityTrackingSection) entityTrackingSection).<Entity>getCollection()) {
                                // Check if the entity is not an Armor Stand (or any other type you want to exclude)
                                if (!(entity instanceof ArmorStandEntity) && entity.getBoundingBox().intersects(box) && predicate.test(entity)) {
                                    return true;
                                }
                            }
                        }
        ...
        // Slow implementation that loops through every entity if for some reason the EntityLookup implementation is changed
        AtomicBoolean found = new AtomicBoolean(false);

        entityLookup.forEachIntersects(box, entity -> {
            if (!found.get() && !(entity instanceof ArmorStandEntity) && predicate.test(entity)) {
                found.set(true);
            }
        });
