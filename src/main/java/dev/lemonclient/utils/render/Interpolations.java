...
public class Interpolations {
     ...
     */
    private static final long lastNanoFrame = System.nanoTime();
    ...
    public static float interpBezier(float startValue, float endValue,
        ...
        double result = startValue + (endValue - startValue) * interpolateCurve(fraction) * Utils.frameTime * 100;
     ...
     * @return curve value
     */
    private static float interpolateCurve(float t) {
        float clampValue;
    ...
    public static Color getColorAnimationState(Color animation, Color finalState, double speed) {
        float add = (float) (Utils.frameTime * 10 * speed);
    ...
    public static float interpLinear(float now, float end, float interpolation) {
        float add = (float) (Utils.frameTime * 10 * interpolation);
        if (now < end) {
            if (now + add < end)
                now += add;
            else
