...
public class Starscript {
    ...
    public static void init() {
        ...
        ss.set("fps", () -> Value.number(IMinecraftClient.getFps()));
        ss.set("ping", Starscript::ping);
        ...
        // Meteor
        ss.set("client", new ValueMap()
            .set("name", LemonClient.NAME)
            .set("version", LemonClient.VERSION != null ? (LemonClient.DEV_BUILD.isEmpty() ? LemonClient.VERSION.toString() : LemonClient.VERSION + " " + LemonClient.DEV_BUILD) : "")
            .set("modules", () -> Value.number(Modules.get().getAll().size()))
            .set("active_modules", () -> Value.number(Modules.get().getActive().size()))
            .set("is_module_active", Starscript::isModuleActive)
            .set("get_module_info", Starscript::getModuleInfo)
            .set("prefix", Starscript::getMeteorPrefix)
        );
        ...
        if (BaritoneUtils.IS_AVAILABLE) {
            ss.set("baritone", new ValueMap()
                ...
                .set("distance_to_goal", Starscript::baritoneDistanceToGoal)
                .set("process", Starscript::baritoneProcess)
                .set("process_name", Starscript::baritoneProcessName)
                .set("eta", Starscript::baritoneETA)
            );
        }
        ...
        ss.set("player", new ValueMap()
            ...
            .set("breaking_progress", () -> Value.number(mc.interactionManager != null ? ((IClientPlayerInteractionManager) mc.interactionManager).getBreakingProgress() : 0))
            .set("biome", Starscript::biome)
            ...
            .set("hand_or_offhand", Starscript::handOrOffhand)
            .set("get_item", Starscript::getItem)
            .set("count_items", Starscript::countItems)
            ...
            .set("has_potion_effect", Starscript::hasPotionEffect)
            .set("get_potion_effect", Starscript::getPotionEffect)

            .set("get_stat", Starscript::getStat)
        );

        // Crosshair target
        ss.set("crosshair_target", new ValueMap()
            .set("type", Starscript::crosshairType)
            .set("value", Starscript::crosshairValue)
        );
    ...
    private static String getCallerName() {
        ...
        for (int i = 1; i < elements.length; i++) {
            ...
            if (name.startsWith(meteordevelopment.starscript.Starscript.class.getPackageName())) continue;
            if (name.equals(Starscript.class.getName())) continue;
    ...
    private static Value hasPotionEffect(meteordevelopment.starscript.Starscript ss, int argCount) {
    ...
    private static Value getPotionEffect(meteordevelopment.starscript.Starscript ss, int argCount) {
    ...
    private static Value getStat(meteordevelopment.starscript.Starscript ss, int argCount) {
    ...
    private static Value getModuleInfo(meteordevelopment.starscript.Starscript ss, int argCount) {
        if (argCount != 1) ss.error("client.get_module_info() requires 1 argument, got %d.", argCount);

        Module module = Modules.get().get(ss.popString("First argument to client.get_module_info() needs to be a string."));
    ...
    private static Value isModuleActive(meteordevelopment.starscript.Starscript ss, int argCount) {
        if (argCount != 1) ss.error("client.is_module_active() requires 1 argument, got %d.", argCount);

        Module module = Modules.get().get(ss.popString("First argument to client.is_module_active() needs to be a string."));
        return Value.bool(module != null && module.isActive());
    }

    private static Value getItem(meteordevelopment.starscript.Starscript ss, int argCount) {
    ...
    private static Value countItems(meteordevelopment.starscript.Starscript ss, int argCount) {
    ...
    public static Identifier popIdentifier(meteordevelopment.starscript.Starscript ss, String errorMessage) {
        try {
