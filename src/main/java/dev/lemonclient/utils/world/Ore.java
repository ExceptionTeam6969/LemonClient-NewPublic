...
import dev.lemonclient.mixin.ICountPlacementModifier;
import dev.lemonclient.mixin.IHeightRangePlacementModifier;
import dev.lemonclient.mixin.IRarityFilterPlacementModifier;
    ...
    private Ore(PlacedFeature feature, int step, int index, Setting<Boolean> active, Color color) {
        ...
        for (PlacementModifier modifier : feature.placementModifiers()) {
            if (modifier instanceof CountPlacementModifier) {
                this.count = ((ICountPlacementModifier) modifier).getCount();

            } else if (modifier instanceof HeightRangePlacementModifier) {
                this.heightProvider = ((IHeightRangePlacementModifier) modifier).getHeight();

            } else if (modifier instanceof RarityFilterPlacementModifier) {
                this.rarity = ((IRarityFilterPlacementModifier) modifier).getChance();
            }
        }
