...
public class DamageUtils {
    ...
    public static void init() {
        LemonClient.EVENT_BUS.subscribe(DamageUtils.class);
    }
    ...
    public static float getExplosionDamage(Vec3d explosionPos, PlayerEntity target) {
        try {
            ...
            ((dev.lemonclient.mixin.IExplosion) explosion).setX(explosionPos.x);
            ((dev.lemonclient.mixin.IExplosion) explosion).setY(explosionPos.y);
            ((dev.lemonclient.mixin.IExplosion) explosion).setZ(explosionPos.z);

            if (((dev.lemonclient.mixin.IExplosion) explosion).getWorld() != mc.world)
                ((dev.lemonclient.mixin.IExplosion) explosion).setWorld(mc.world);
