...
public class DamageInfo {
    ...
    public static void init() {
        LemonClient.EVENT_BUS.subscribe(DamageInfo.class);
    }
    ...
    public static double crystalDamage(Entity entity, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
        if (entity == null) return 0;

        ((IVec3d) vec3d).set(entity.getPos().x, entity.getPos().y, entity.getPos().z);
        if (predictMovement)
            ((IVec3d) vec3d).set(vec3d.x + entity.getVelocity().x, vec3d.y + entity.getVelocity().y, vec3d.z + entity.getVelocity().z);

        double modDistance = Math.sqrt(vec3d.squaredDistanceTo(crystal));
        if (modDistance > 12) return 0;

        double exposure = getExposure(crystal, entity, predictMovement, raycastContext, obsidianPos, ignoreTerrain);
        double impact = (1 - (modDistance / 12)) * exposure;
        double damage = ((impact * impact + impact) / 2 * 7 * (6 * 2) + 1);

        damage = getDamageForDifficulty(damage);
        if (entity instanceof LivingEntity ent) {
            damage = DamageUtil.getDamageLeft((float) damage, (float) ent.getArmor(), (float) ent.getAttributeInstance(EntityAttributes.GENERIC_ARMOR_TOUGHNESS).getValue());
            damage = resistanceReduction(ent, damage);
        }

        ((IExplosion) explosion).set(crystal, 6, false);
        damage = blastProtReduction(entity, damage, explosion);

        return damage < 0 ? 0 : damage;
    }

    public static double crystalDamage(Entity entity, Box bb, Vec3d crystal, BlockPos obsidianPos, boolean ignoreTerrain) {
        if (entity == null) return 0;
        ...
        double exposure = getExposure(crystal, entity, bb, raycastContext, obsidianPos, ignoreTerrain);
        double impact = (1 - (modDistance / 12)) * exposure;
        double damage = ((impact * impact + impact) / 2 * 7 * (6 * 2) + 1);
