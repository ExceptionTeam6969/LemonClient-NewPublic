...
import dev.lemonclient.events.entity.player.PreRotationEvent;
...
import dev.lemonclient.events.rotate.PitchDiffEvent;
import dev.lemonclient.events.rotate.YawDiffEvent;
import dev.lemonclient.events.world.TickEvent;
import dev.lemonclient.managers.Managers;
import dev.lemonclient.systems.modules.Modules;
import dev.lemonclient.systems.modules.settings.RotationSettings;
import dev.lemonclient.utils.SettingUtils;
import dev.lemonclient.utils.player.CombatInfo;
import dev.lemonclient.utils.player.NCPRaytracer;
...
public class RotationManager {
    ...
    private boolean strafeFix, silentStrafe;
    ...
    @EventHandler(priority = EventPriority.HIGHEST)
    private void onPitchDiff(PitchDiffEvent event) {
        event.pitchDiff = currentDir[1] - lastDir[1];
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    private void onYawDiff(YawDiffEvent event) {
        event.yawDiff = currentDir[0] - lastDir[0];
    }
    ...
    @EventHandler(priority = EventPriority.HIGHEST)
    private void onStrafe(StrafeMoveEvent event) {
        if (mc.player == null) return;
        if (settings == null) settings = Modules.get().get(RotationSettings.class);
        this.strafeFix = settings.strafeFix.get();
        this.silentStrafe = settings.silentStrafe.get();
        ...
        if (shouldRotate) {
            if (strafeFix) {
                ...
                event.yaw = MathHelper.lerpAngleDegrees(mc.getTickDelta(), prevDir[0], currentDir[0]);
            }
        }
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    private void onStrafe(StrafeJumpEvent event) {
        if (mc.player == null) return;
    ...
    private void onPreRotate() {
        LemonClient.EVENT_BUS.post(PreRotationEvent.INSTANCE);
    }
    ...
    private void updateNextRotation() {
        if (shouldRotate) {
            ...
            if (strafeFix) {
                next = RotationUtils.fixedSensitivity(next, lastDir, mc.options.getMouseSensitivity().getValue());
            }
    ...
    public boolean start(BlockPos pos, Box box, Vec3d vec, double p, RotationType type, long key) {
        ...
        if (p < priority || key == this.key || (p == priority && (!(target instanceof BoxTarget) || SettingUtils.rotationCheck(((BoxTarget) target).box, type)))) {
            ...
            this.key = key;
            target = pos != null ? new BoxTarget(pos, vec != null ? vec : CombatInfo.getMiddle(box), p, type) : new BoxTarget(box, vec != null ? vec : CombatInfo.getMiddle(box), p, type);
            timer = settings.time(type);
        }
    ...
    public boolean start(Box box, double p, RotationType type, long key) {
        return start(box, CombatInfo.getMiddle(box), p, type, key);
    }
