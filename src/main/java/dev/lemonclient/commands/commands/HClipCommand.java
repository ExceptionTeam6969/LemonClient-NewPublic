...
public class HClipCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("blocks", DoubleArgumentType.doubleArg()).executes(context -> {
            ...
            Vec3d forward = Vec3d.fromPolar(0, LemonClient.mc.player.getYaw()).normalize();

            if (LemonClient.mc.player.hasVehicle()) {
                Entity vehicle = LemonClient.mc.player.getVehicle();
                vehicle.setPosition(vehicle.getX() + forward.x * blocks, vehicle.getY(), vehicle.getZ() + forward.z * blocks);
            }

            LemonClient.mc.player.setPosition(LemonClient.mc.player.getX() + forward.x * blocks, LemonClient.mc.player.getY(), LemonClient.mc.player.getZ() + forward.z * blocks);
