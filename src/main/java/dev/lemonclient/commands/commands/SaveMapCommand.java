...
import dev.lemonclient.mixin.IMapRenderer;
...
public class SaveMapCommand extends Command {
    ...
    private void saveMap(ItemStack map, MapState state, String path, int scale) {
        ...
        MapRenderer mapRenderer = LemonClient.mc.gameRenderer.getMapRenderer();
        MapRenderer.MapTexture texture = ((IMapRenderer) mapRenderer).invokeGetMapTexture(FilledMapItem.getMapId(map), state);
    ...
    private MapState getMapState() {
        ...
        return FilledMapItem.getMapState(FilledMapItem.getMapId(map), LemonClient.mc.world);
    }
    ...
    private ItemStack getMap() {
        ItemStack itemStack = LemonClient.mc.player.getMainHandStack();
        if (itemStack.getItem() == Items.FILLED_MAP) return itemStack;

        itemStack = LemonClient.mc.player.getOffHandStack();
        if (itemStack.getItem() == Items.FILLED_MAP) return itemStack;
