...
import dev.lemonclient.systems.modules.movement.NoFall;
import dev.lemonclient.systems.modules.player.AntiHunger;
...
public class DamageCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("damage", IntegerArgumentType.integer(1, 7)).executes(context -> {
            ...
            if (LemonClient.mc.player.getAbilities().invulnerable) {
    ...
    private void damagePlayer(int amount) {
        ...
        Vec3d pos = LemonClient.mc.player.getPos();
    ...
    private void sendPositionPacket(double x, double y, double z, boolean onGround) {
        LemonClient.mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(x, y, z, onGround));
    }
}
