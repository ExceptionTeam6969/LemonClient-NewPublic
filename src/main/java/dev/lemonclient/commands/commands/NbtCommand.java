...
import dev.lemonclient.commands.arguments.CompoundNbtTagArgumentType;
...
public class NbtCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(literal("add").then(argument("nbt", CompoundNbtTagArgumentType.create()).executes(s -> {
            ItemStack stack = LemonClient.mc.player.getInventory().getMainHandStack();
        ...
        builder.then(literal("copy").executes(context -> {
            ...
            if (stack == null) {
                ...
                LemonClient.mc.keyboard.setClipboard(tag.toString());
        ...
        builder.then(literal("paste").executes(context -> {
            ...
            if (validBasic(stack)) {
                stack.setNbt(new CompoundNbtTagArgumentType().parse(new StringReader(LemonClient.mc.keyboard.getClipboard())));
                setStack(stack);
            }
    ...
    private void setStack(ItemStack stack) {
        LemonClient.mc.player.networkHandler.sendPacket(new CreativeInventoryActionC2SPacket(36 + LemonClient.mc.player.getInventory().selectedSlot, stack));
    }

    private boolean validBasic(ItemStack stack) {
        if (!LemonClient.mc.player.getAbilities().creativeMode) {
