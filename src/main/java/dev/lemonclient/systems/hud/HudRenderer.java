...
import dev.lemonclient.renderer.text.CustomTextRenderer;
import dev.lemonclient.renderer.text.Font;
import dev.lemonclient.renderer.text.VanillaTextRenderer;
...
public class HudRenderer {
        ...
        if (hud.hasCustomFont()) {
            ...
            for (Iterator<FontHolder> it = fontsInUse.values().iterator(); it.hasNext(); ) {
                ...
                if (fontHolder.visited) {
                    ...
                    fontHolder.getMesh().render(null, 1);
                } else {
                    it.remove();
    ...
    public void quadRounded(double x, double y, double width, double height, double r, Color color) {
        Renderer2D.COLOR.quadRounded(x, y, width, height, r, color);
    }
    ...
    public double textWidth(String text, boolean shadow, double scale) {
        ...
        if (hud.hasCustomFont()) {
            double width = getFont(scale).getWidth(text, (scale == -1 ? hud.getTextScale() : scale), text.length());
            return (width + (shadow ? 1 : 0)) + (shadow ? 1 : 0);
        }
    ...
    private static class FontHolder {
        ...
        public Mesh getMesh() {
            if (mesh == null)
                mesh = new ShaderMesh(Shaders.TEXT, DrawMode.Triangles, Mesh.Attrib.Vec2, Mesh.Attrib.Vec2, Mesh.Attrib.Color);
