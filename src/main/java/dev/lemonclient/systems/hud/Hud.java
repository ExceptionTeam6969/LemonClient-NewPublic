...
import dev.lemonclient.systems.hud.elements.*;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("LemonClient");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the client hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    public void init() {
        ...
        register(StarscriptTextHud.INFO);
        register(TabGuiHud.INFO);
        register(TargetHud.INFO);
        register(ToastNotificationsHud.INFO);
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(StarscriptTextHud.WATERMARK, 4, 4, XAnchor.Left, YAnchor.Top);
        add(StarscriptTextHud.FPS, 4, 4 + h, XAnchor.Left, YAnchor.Top);
        add(StarscriptTextHud.TPS, 4, 4 + h * 2, XAnchor.Left, YAnchor.Top);
        add(StarscriptTextHud.PING, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(StarscriptTextHud.SPEED, 4, 4 + h * 4, XAnchor.Left, YAnchor.Top);

        // Top Right
        add(ActiveModulesHud.INFO, -4, 4, XAnchor.Right, YAnchor.Top);

        // Bottom Right
        add(StarscriptTextHud.POSITION, -4, -4, XAnchor.Right, YAnchor.Bottom);
        add(StarscriptTextHud.OPPOSITE_POSITION, -4, -4 - h, XAnchor.Right, YAnchor.Bottom);
        add(StarscriptTextHud.ROTATION, -4, -4 - h * 2, XAnchor.Right, YAnchor.Bottom);
    }
    ...
    private void onRender(Render2DEvent event) {
        ...
        if ((mc.options.hudHidden || mc.options.debugEnabled) && !HudEditorScreen.isOpen())
            return;
