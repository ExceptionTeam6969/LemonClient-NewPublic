...
public class PlayerRadarHud extends HudElement {
    ...
    public void tick(HudRenderer renderer) {
        ...
        if (LemonClient.mc.world == null) {
            setSize(width, height);
            return;
        }

        for (PlayerEntity entity : getPlayers()) {
            if (entity.equals(LemonClient.mc.player)) continue;
            if (!friends.get() && Friends.get().isFriend(entity)) continue;

            String text = entity.getEntityName();
            if (distance.get())
                text += String.format("(%sm)", Math.round(LemonClient.mc.getCameraEntity().distanceTo(entity)));
    ...
    public void render(HudRenderer renderer) {
        ...
        if (LemonClient.mc.world == null) return;
        ...
        for (PlayerEntity entity : getPlayers()) {
            ...
            if (distance.get()) {
                distanceText = String.format("(%sm)", Math.round(LemonClient.mc.getCameraEntity().distanceTo(entity)));
    ...
    private List<AbstractClientPlayerEntity> getPlayers() {
        players.clear();
        players.addAll(LemonClient.mc.world.getPlayers());
        if (players.size() > limit.get()) players.subList(limit.get() - 1, players.size() - 1).clear();
        players.sort(Comparator.comparingDouble(e -> e.squaredDistanceTo(LemonClient.mc.getCameraEntity())));
