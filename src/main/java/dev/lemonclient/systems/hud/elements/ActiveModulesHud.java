...
public class ActiveModulesHud extends HudElement {
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (Module module : Modules.get().getActive()) {
            if (!hiddenModules.get().contains(module) && !modules.contains(module)) modules.add(module);
        }

        if (modules.isEmpty()) {
            if (isInEditor()) {
                setSize(renderer.textWidth("Array List", shadow.get(), getScale()), renderer.textHeight(shadow.get(), getScale()));
            }
            return;
        }
        ...
        for (Module module : modules) {
            if (!module.isActive()) {
                module.guiVector.z = this.x + this.getWidth();
                if (Math.abs(module.guiVector.x - module.guiVector.z) == 0) {
    ...
    public void render(HudRenderer renderer) {
        ...
        if (modules.isEmpty()) {
            if (isInEditor()) {
                renderer.text("Array List", x, y, WHITE, shadow.get(), getScale());
            }
            return;
        }
        ...
        for (int i = 0; i < modules.size(); i++) {
            ...
            Module module = modules.get(i);
            module.guiVector.z = x + offset;
            module.guiVector.w = y;

            renderModule(renderer, modules, i, module.guiVector.x, module.guiVector.y);

            prevX = x + offset;
            if (!isInEditor()) {
                module.guiVector.x = moveModule(module.guiVector.x, module.guiVector.z, moveSpeed.get());
                module.guiVector.y = moveModule(module.guiVector.y, module.guiVector.w, moveSpeed.get());
            } else {
                module.guiVector.x = module.guiVector.z;
                module.guiVector.y = module.guiVector.w;
            }
    ...
    public double moveModule(double start, double end, double sp) {
        if (start == -1) {
            start = end;
        }

        double speed = (end - start) * sp;

        if (speed > 0) {
            speed = Math.max(sp, speed);
            speed = Math.min(end - start, speed);
        } else if (speed < 0) {
            speed = Math.min(-sp, speed);
