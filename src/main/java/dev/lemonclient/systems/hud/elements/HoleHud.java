...
public class HoleHud extends HudElement {
    ...
    private Direction get(Facing dir) {
        ...
        return Direction.fromRotation(MathHelper.wrapDegrees(LemonClient.mc.player.getYaw() + dir.offset));
    }

    private void drawBlock(HudRenderer renderer, Direction dir, double x, double y) {
        Block block = dir == Direction.DOWN ? Blocks.OBSIDIAN : LemonClient.mc.world.getBlockState(LemonClient.mc.player.getBlockPos().offset(dir)).getBlock();
        ...
        ((IWorldRenderer) LemonClient.mc.worldRenderer).getBlockBreakingInfos().values().forEach(info -> {
            if (info.getPos().equals(LemonClient.mc.player.getBlockPos().offset(dir))) {
