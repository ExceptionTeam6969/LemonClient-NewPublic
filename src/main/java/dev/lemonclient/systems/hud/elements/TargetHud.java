...
public class TargetHud extends HudElement {
    public static final HudElementInfo<TargetHud> INFO = new HudElementInfo<>(Hud.GROUP, "Target Hud", "A target hud the fuck you thinkin bruv.", TargetHud::new);
    ...
    public void render(HudRenderer renderer) {
        if (mode.get() == Mode.Normal) {
            ...
            // Background
            Render3DUtils.rounded(stack, 15, 15, width - 30, height - 30, 15, 10, bgColor.get().getPacked());
            ...
            // Name
            Render3DUtils.text(renderName, stack, 60, 20, textColor.get().getPacked());

            // Ping
            Render3DUtils.text(Math.round(renderPing) + "ms", stack, 60, 30, textColor.get().getPacked());

            // Health
            Render3DUtils.text(String.valueOf(Math.round((renderHealth) * 10) / 10f), stack, 20, 81 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            ...
            // Health Bar
            if (barAnimation > 0) {
                Render3DUtils.rounded(stack, barStart, 80, MathHelper.clamp(barAnimation / 20, 0, 1) * (width - 30 - barStart), 2, 3, 10, healthColor.get().getPacked());
            }
            if (barAnimation > 20) {
                Render3DUtils.rounded(stack, barStart, 80, MathHelper.clamp((barAnimation - 20) / 16, 0, 1) * (width - 30 - barStart), 2, 3, 10, absorptionColor.get().getPacked());
            }
        }
        if (mode.get() == Mode.ExhibitionOld) {
            ...
            // Background
            Render3DUtils.quad(stack, 0, 0, width, height, bgColor.get().getPacked());

            // Face
            Render3DUtils.quad(stack, 1, 1, 58, 58, new Color(102, 102, 102, 255).getPacked());

            drawFace(renderer, scale.get().floatValue(), x, y, 0);

            // Name
            stack.scale(2.0f, 2.0f, 1);
            Render3DUtils.text(renderName, stack, 33, 2, textColor.get().getPacked());

            // Health
            stack.scale(0.5f, 0.5f, 1);
            Render3DUtils.text(Math.round((renderHealth) * 10) / 10f + " Dist: " + Math.round(mc.player.distanceTo(target) * 10) / 10f, stack, 66, 35 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            ...
            for (int i = 0; i < 10; i++) {
                Render3DUtils.quad(stack, 33 + i * 8, 11, 3 * Math.min(progress, 2), 3, new Color(204, 204, 0, 255).getPacked());
                progress -= 2;
            ...
            stack.scale(0.5f, 0.5f, 1);
            // Misc info
            Render3DUtils.text("Yaw: " + Math.round((target.getYaw()) * 10) / 10f + " Pitch: " + Math.round(target.getPitch() * 10) / 10f + " BodyYaw: " + Math.round((target.getBodyYaw()) * 10) / 10f, stack, 66, 45 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            Render3DUtils.text("TOG: " + (tog.getOrDefault(target, 0)) + " HURT: " + ((target.hurtTime) * 10) / 10f + " TE: " + target.age, stack, 66, 55 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
        }
        if (mode.get() == Mode.Exhibition) {
            ...
            // Background
            Render3DUtils.quad(stack, -2, -2, width + 4, height + 4, new Color(52, 52, 52, 255).getPacked());
            Render3DUtils.quad(stack, -1, -1, width + 2, height + 2, new Color(32, 32, 32, 255).getPacked());
            Render3DUtils.quad(stack, 0, 0, width, height, new Color(52, 52, 52, 255).getPacked());

            //PlayerModel

            // Name
            stack.scale(1.5f, 1.5f, 1);
            Render3DUtils.text(renderName, stack, 41, 2, textColor.get().getPacked());

            // Health and Distance
            stack.scale(0.5f, 0.5f, 1);
            Render3DUtils.text(Math.round((renderHealth) * 10) / 10f + " Dist: " + Math.round(mc.player.distanceTo(target) * 10) / 10f, stack, 83, 40 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            ...
            for (int i = 0; i < 10; i++) {
                Render3DUtils.quad(stack, 41 + i * 8, 12, 3 * Math.min(progress, 2), 3, new Color(204, 204, 0, 255).getPacked());
                progress -= 2;
