package dev.lemonclient.systems.waypoints;
...
public class Waypoints extends System<Waypoints> implements Iterable<Waypoint> {
    ...
    @Override
    public void init() {
        File iconsFolder = new File(new File(LemonClient.FOLDER, "waypoints"), "icons");
        iconsFolder.mkdirs();
    ...
    public File getFile() {
        ...
        return new File(new File(LemonClient.FOLDER, "waypoints"), Utils.getFileWorldName() + ".nbt");
    }
    ...
    private void copyIcon(File file) {
        StreamUtils.copy(Waypoints.class.getResourceAsStream("/assets/" + LemonClient.MOD_ID + "/textures/icons/waypoints/" + file.getName()), file);
    }
