...
public class StashFinder extends Module {
    public StashFinder() {
        super(Categories.World, "Stash Finder", "Searches loaded chunks for storage blocks. Saves to <your minecraft folder>/lemon-client");
    }
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            ...
            if (sendNotifications.get() && (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk))) {
                switch (notificationMode.get()) {
                    ...
                    case Toast ->
                        mc.getToastManager().add(new ToastSystem(Items.CHEST, null, title, null, "Found Stash!"));
                    case Both -> {
                        info("Found stash at (highlight)%s(default), (highlight)%s(default).", chunk.x, chunk.z);
                        mc.getToastManager().add(new ToastSystem(Items.CHEST, null, title, null, "Found Stash!"));
                    }
    ...
    private File getJsonFile() {
        return new File(new File(new File(LemonClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.json");
    }

    private File getCsvFile() {
        return new File(new File(new File(LemonClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.csv");
    }
    ...
    private static class ChunkScreen extends WindowScreen {
        ...
        public void initWidgets() {
            ...
            t.add(theme.label(String.valueOf(chunk.getTotal())));
            t.row();
            ...
            t.add(theme.label(String.valueOf(chunk.chests)));
            t.row();

            t.add(theme.label("Barrels:"));
            t.add(theme.label(String.valueOf(chunk.barrels)));
            t.row();

            t.add(theme.label("Shulkers:"));
            t.add(theme.label(String.valueOf(chunk.shulkers)));
            t.row();

            t.add(theme.label("Ender Chests:"));
            t.add(theme.label(String.valueOf(chunk.enderChests)));
            t.row();

            t.add(theme.label("Furnaces:"));
            t.add(theme.label(String.valueOf(chunk.furnaces)));
            t.row();

            t.add(theme.label("Dispensers and droppers:"));
            t.add(theme.label(String.valueOf(chunk.dispensersDroppers)));
            t.row();

            t.add(theme.label("Hoppers:"));
            t.add(theme.label(String.valueOf(chunk.hoppers)));
        }
    }
}
