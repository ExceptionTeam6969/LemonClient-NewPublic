...
import static dev.lemonclient.utils.render.Render3DUtils.*;
...
public class AutoCrystal extends Module {
    public AutoCrystal() {
        super(Categories.Combat, "Auto Crystal", "Automatically place and attack crystals in less strict anti-cheat servers.");
    }
    ...
    private final Setting<Boolean> placement = sgPlace.add(new BoolSetting.Builder().name("1.12-placement").description("Uses 1.12 crystal placement.").defaultValue(false).build());
    ...
    private final Setting<Integer> smoothness = sgRender.add(new IntSetting.Builder().name("smoothness").description("How smoothly the render should move around.").defaultValue(10).min(0).sliderMax(20).visible(() -> renderMode.get() == RenderMode.Smooth).build());
    ...
    public final BlockPos.Mutable renderPos = new BlockPos.Mutable();
    ...
    private void onPreTickLast(TickEvent.Pre event) {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && lastRotationTimer < getLastRotationStopDelay() && !didRotateThisTick) {
            Managers.ROTATION.start(isLastRotationPos ? Rotations.getYaw(lastRotationPos) : lastYaw, isLastRotationPos ? Rotations.getPitch(lastRotationPos) : lastPitch, priority, RotationType.BlockPlace, Objects.hash(name + "placing"));
        }
    }
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof EntityVelocityUpdateS2CPacket packet
            ...
            ((IEntityVelocityUpdateS2CPacket) packet).setX((int) (velX * 8000 + bestTarget.getVelocity().x * 8000));
            ((IEntityVelocityUpdateS2CPacket) packet).setY((int) (velY * 8000 + bestTarget.getVelocity().y * 8000));
            ((IEntityVelocityUpdateS2CPacket) packet).setZ((int) (velZ * 8000 + bestTarget.getVelocity().z * 8000));
        }
    }
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            double y = bp.getY() + 1;
            double z = bp.getZ();
            ((IBox) box).set(x, y, z, x + 1, y + (placement.get() ? 1 : 2), z + 1);
        ...
        BlockIterator.after(() -> {
            ...
            if (SettingUtils.shouldRotate(RotationType.BlockPlace))
                Managers.ROTATION.start(BlockPos.ofFloored(vec3d), priority, RotationType.BlockPlace, Objects.hash(name + "placing"));
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        if (supportBlock == null) {
            ...
            placeBlock(supportBlock, item, true);
            InvUtils.swapBack();
...
/*        // Fake players
    ...
    private void onRender(Render3DEvent event) {
        Render3DUtils.RenderInfo ri = new Render3DUtils.RenderInfo(event, renderMode.get(), shapeMode.get());
        ...
        if (renderMode.get() == RenderMode.Smooth) {
            ...
            if (renderBoxTwo == null) renderBoxTwo = new Box(renderPos);
            else ((IBox) renderBoxTwo).set(renderPos);
    ...
    public boolean shouldFacePlace() {
        ...
        for (PlayerEntity target : targets) {
            if (!HoleUtils.inHole(target) || isTrapped(target)) return false;
    ...
    public EndCrystalEntity getSbCrystal(BlockPos blockPos) {
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof EndCrystalEntity crystal && crystal.getBlockPos().equals(blockPos)) return crystal;
        }
    ...
    public class RenderBlock {
        ...
        public void render(Render3DUtils.RenderInfo ri, Color sides, Color lines) {
            ...
            Render3DUtils.render(ri, this.pos, sides, lines, height.get());
            sides.a = preSideA;
            lines.a = preLineA;
        }
