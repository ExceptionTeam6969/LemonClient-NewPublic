...
public class SelfTrap extends Module {
    public SelfTrap() {
        super(Categories.Combat, "Self Trap", "Traps yourself with blocks.");
    }
    ...
    //--------------------Placing--------------------//
    private final Setting<Boolean> attack = sgPlacing.add(new BoolSetting.Builder()
        .name("Attack")
        .description("Attacks crystals blocking selftrap.")
        .defaultValue(true)
        .build()
    );
                ...
                if (obsidian >= 0) {
                    if (hand == null) {
                        switch (switchMode.get()) {
                            ...
                            case PickSilent ->
                                obsidian = InvUtils.pickSwitch(inventory.slot()) ? inventory.count() : -1;
                            case InvSwitch -> obsidian = InvUtils.invSwitch(inventory.slot()) ? inventory.count() : -1;
                        }
                    }
        ...
        if (mc.player != null && mc.world != null) {
            for (int x = size[0] - 1; x <= size[1] + 1; x++) {
                for (int z = size[2] - 1; z <= size[3] + 1; z++) {
                    ...
                    boolean ignore = isX && !isZ ? (!BlockUtils.replaceable(pos.add(CombatInfo.closerToZero(x), 0, z)) || placed.contains(pos.add(CombatInfo.closerToZero(x), 0, z))) :
                        !isX && isZ && (!BlockUtils.replaceable(pos.add(x, 0, CombatInfo.closerToZero(z))) || placed.contains(pos.add(x, 0, CombatInfo.closerToZero(z))));
                    BlockPos bPos = null;
