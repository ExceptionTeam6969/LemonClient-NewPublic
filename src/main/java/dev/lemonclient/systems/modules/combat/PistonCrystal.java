...
public class PistonCrystal extends Module {
    public PistonCrystal() {
        super(Categories.Combat, "Piston Crystal", "Pushes crystals into your enemies to deal massive damage.");
    }
    ...
    private void updateAttack() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Attacking) && !Managers.ROTATION.start(crystal.getBoundingBox(), priority - 0.1, RotationType.Attacking, Objects.hash(name + "attack")))
            return;
    ...
    private void updateCrystal() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Interact) && !Managers.ROTATION.start(crystalPos.down(), priority, RotationType.Interact, Objects.hash(name + "crystal")))
            return;
        ...
        if (hand == null) {
            switch (crystalSwitch.get()) {
                ...
                case PickSilent -> switched = InvUtils.pickSwitch(InvUtils.find(Items.END_CRYSTAL).slot());
                case InvSwitch -> switched = InvUtils.invSwitch(InvUtils.find(Items.END_CRYSTAL).slot());
            }
        }
    ...
    private void updateRedstone() {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(redstoneData.pos(), priority, RotationType.BlockPlace, Objects.hash(name + "restone")))
            return;
    ...
    private void updateFire() {
        ...
        for (int x = (crystalDir.getOpposite().getOffsetX() == 0 ? -1 : Math.min(0, crystalDir.getOffsetX())); x <= (crystalDir.getOpposite().getOffsetX() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetX())); x++) {
            for (int y = 0; y <= 1; y++) {
                for (int z = (crystalDir.getOpposite().getOffsetZ() == 0 ? -1 : Math.min(0, crystalDir.getOffsetZ())); z <= (crystalDir.getOpposite().getOffsetZ() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetZ())); z++) {
                    ...
                    if (!BlockUtils.solid(pos.down())) continue;
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(data.pos(), priority, RotationType.BlockPlace, Objects.hash(name + "fire")))
            return;
        ...
        if (hand == null) {
            switch (fireSwitch.get()) {
                ...
                case PickSilent -> switched = InvUtils.pickSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
                case InvSwitch -> switched = InvUtils.invSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
            }
        }
    ...
    private List<BlockPos> pistonBlocks(BlockPos pos, Direction dir) {
        ...
        return blocks.stream().filter(b -> {
            ...
            return BlockUtils.replaceable(b);
        }).toList();
    }
    ...
    private void redstonePos(BlockPos pos, Direction pDir, BlockPos cPos) {
        ...
        if (redstone.get() == Redstone.Torch) {
            for (Direction direction : Direction.values()) {
                ...
                if (!BlockUtils.replaceable(position) && !(mc.world.getBlockState(position).getBlock() instanceof RedstoneTorchBlock) && !(mc.world.getBlockState(position).getBlock() instanceof FireBlock)) {
                    continue;
                }
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (Box.from(new BlockBox(position)).intersects(CombatInfo.getCrystalBox(cPos))) {
                continue;
            }
