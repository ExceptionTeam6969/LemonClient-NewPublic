...
import dev.lemonclient.mixin.IInteractEntityC2SPacket;
...
import dev.lemonclient.systems.modules.misc.Suicide;
...
import dev.lemonclient.utils.entity.LemonEntityUtils;
...
public class AutoCrystalPlus extends Module {
    public AutoCrystalPlus() {
        super(Categories.Combat, "Auto Crystal+", "Automatically place and attack crystals in strict anti-cheat servers.");
    }
    ...
    private final Setting<SwitchMode> switchMode = sgSwitch.add(new EnumSetting.Builder<SwitchMode>()
        ...
        .defaultValue(SwitchMode.Simple)
        .build()
    );
    ...
    private final Setting<Integer> decimal = sgRender.add(new IntSetting.Builder()
        .name("Decimal")
        .defaultValue(1)
        .min(1)
        .sliderRange(1, 10)
    ...
    private final Setting<Double> renderTime = sgRender.add(new DoubleSetting.Builder()
        ...
        .min(0)
        .sliderRange(0, 10)
        .visible(() -> renderMode.get().equals(RenderMode.Earthhack) || renderMode.get().equals(RenderMode.Future) || renderDmg.get())
        .build()
    );
    private final Setting<FadeMode> fadeMode = sgRender.add(new EnumSetting.Builder<FadeMode>()
        ...
        .visible(() -> renderMode.get() == RenderMode.Boze)
        .build()
    );
    ...
    private final Setting<Double> animationSpeed = sgRender.add(new DoubleSetting.Builder()
        ...
        .description("How fast should boze mode box move.")
        .defaultValue(1)
        .min(0)
        .sliderRange(0, 10)
        .visible(() -> renderMode.get().equals(RenderMode.Boze))
        .build()
    );
    ...
    private double selfDmg = 0.0;
    private double bestDmg = 0.0;
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        delta = (System.currentTimeMillis() - lastMillis) / 1000f;
        lastMillis = System.currentTimeMillis();

        if (System.currentTimeMillis() - lastExplosion > 5000) cps = 0;

        infoCps = MathHelper.lerp(delta / 3, infoCps, cps);
        ...
        if (!TimeBomber.shouldBomb()) update();
        checkDelayed();

        // Rendering
        if (renderTargetEsp.get() && bestTarget != null && placePos != null) {
            Render3DUtils.drawJello(event.matrices, bestTarget, color.get());
        }
                ...
                case Boze -> {
                    ...
                    if (renderPos != null) {
                        ...
                        if (r >= 0.001) {
                            ...
                            event.renderer.box(box, new Color(sideColor.get().r, sideColor.get().g, sideColor.get().b, Math.round(sideColor.get().a)), lineColor.get(), shapeMode.get(), 0);
                        }
                    }
                }
                case Future -> {
                    ...
                    if (renderProgress > 0 && renderPos != null) {
                        event.renderer.box(new Box(renderPos.getX(), renderPos.getY() - 1, renderPos.getZ(), renderPos.getX() + 1, renderPos.getY(), renderPos.getZ() + 1),
                            Render2DUtils.injectAlpha(sideColor.get(), (int) Math.round(sideColor.get().a * Math.min(1, renderProgress / fadeTime.get()))),
                            Render2DUtils.injectAlpha(lineColor.get(), (int) Math.round(lineColor.get().a * Math.min(1, renderProgress / fadeTime.get()))), shapeMode.get(), 0);
                    }
                }
                case Earthhack -> {
                    List<BlockPos> toRemove = new ArrayList<>();
                    for (Map.Entry<BlockPos, Double[]> entry : earthMap.entrySet()) {
                        ...
                        if (alpha[0] <= delta) {
                            ...
                            event.renderer.box(box,
                                new Color(sideColor.get().r, sideColor.get().g, sideColor.get().b, (int) Math.round(sideColor.get().a * Math.min(1, alpha[0] / alpha[1]))),
    ...
    private void onRender2D(Render2DEvent event) {
        double renderProgress = this.renderProgress;
        ...
        if (renderDmg.get() && renderProgress > 0 && renderPos != null) {
            Vector3d vec3d = new Vector3d(renderPos.getX() + 0.5, renderPos.getY() - 0.5, renderPos.getZ() + 0.5);
            if (NametagUtils.to2D(vec3d, 1)) {
                ...
                String text = why.format(bestDmg) + " / " + why.format(selfDmg);

                font.render(text, -(font.getWidth(text) / 2.0), -font.getHeight(), Render2DUtils.injectAlpha(damageColor.get(), (int) Math.round(damageColor.get().a * Math.min(1, renderProgress / fadeTime.get()))));
    ...
    private void onSend(PacketEvent.Send event) {
        if (mc.player != null && mc.world != null) {
            ...
            if (TimeBomber.shouldBomb()) return;
            if (event.packet instanceof PlayerInteractBlockC2SPacket packet) {
                ...
                blocked.add(CombatInfo.getCrystalBox(packet.getBlockHitResult().getBlockPos().up()));
    ...
    private void update() {
        ...
        switch (switchMode.get()) {
            ...
            case Gapple -> {
                int gapSlot = InvUtils.findInHotbar(CombatInfo::isGapple).slot();
                if (mc.options.useKey.isPressed() && Managers.HOLDING.isHolding(Items.END_CRYSTAL, Items.ENCHANTED_GOLDEN_APPLE, Items.GOLDEN_APPLE) && gapSlot >= 0) {
                    if (getHand(CombatInfo::isGapple) == null) {
        ...
        if (expEntity != null) {
            if (multiTaskCheck() && !attackedList.contains(expEntity.getId()) && attackDelayCheck() && existedCheck(expEntity.getBlockPos())) {
    ...
    private boolean startAttackRot() {
        return (Managers.ROTATION.start(expEntity.getBoundingBox(), smartRot.get() ? expEntity.getPos() : null, priority + (!attackedList.contains(expEntity.getId()) && blocksPlacePos(expEntity) ? -0.1 : 0.1), RotationType.Attacking, Objects.hash(name + "attacking")));
    }
    ...
    private void placeCrystal(BlockPos pos, Direction dir, Hand handToUse, int sl, int hsl) {
        if (pos != null && mc.player != null) {
            ...
            if (switched) {
                switch (switchMode.get()) {
                    case PickSilent -> InvUtils.pickSwitch(sl);
                    case Silent -> InvUtils.swap(sl, true);
                    case InvSilent -> InvUtils.invSwitch(sl);
                }
            }
            ...
            if (switched) {
                switch (switchMode.get()) {
                    ...
                    case InvSilent -> InvUtils.invSwapBack();
                }
            }
            if (idPredict.get()) {
    ...
    private void explode(int id, Vec3d vec) {
        attackEntity(id, CombatInfo.getCrystalBox(vec), vec);
    }
    ...
    private BlockPos getPlacePos() {
        int r = (int) Math.ceil(Math.max(SettingUtils.range.crystalPlaceRange.get(), SettingUtils.range.crystalPlaceRangeWalls.get()));
        ...
        for (int x = -r; x <= r; x++) {
            for (int y = -r; y <= r; y++) {
                for (int z = -r; z <= r; z++) {
                    ...
                    if (!air(pos) || !(!SettingUtils.oldCrystals() || air(pos.up())) || !crystalBlock(pos.down()) || blockBroken(pos.down()))
                        continue;
                    ...
                    if (LemonEntityUtils.intersectsWithEntity(box, this::validForIntersect, extHitbox)) continue;

                    // Sets the damage
                    this.bestDmg = result[0][0];
                    this.selfDmg = result[0][2];
    ...
    private double[][] getDmg(Vec3d vec, boolean attack) {
        double self = DamageInfo.crystal(mc.player, extPos.containsKey(mc.player) ? extPos.get(mc.player) : mc.player.getBoundingBox(), vec, ignorePos(attack), ignoreTerrain.get());
        ...
        for (Map.Entry<AbstractClientPlayerEntity, Box> entry : extPos.entrySet()) {
            ...
            double dmg = DamageInfo.crystal(player, box, vec, ignorePos(attack), ignoreTerrain.get());
    ...
    private boolean inPlaceRange(BlockPos pos) {
        return SettingUtils.inCrystalPlaceRange(pos);
    }
