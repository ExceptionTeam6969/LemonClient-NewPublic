...
public class AutoHoleFillPlus extends Module {
    ...
    //--------------------Placing--------------------//
    private final Setting<Boolean> pauseOnEat = sgPlacing.add(new BoolSetting.Builder()
        .name("Pause On Eat")
    ...
    private boolean validPos(BlockPos pos) {
        ...
        if (LemonEntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos)), entity -> !entity.isSpectator() && !(entity instanceof ItemEntity), boxes))
            return false;
    ...
    private boolean validHole(Hole hole) {
        ...
        for (AbstractClientPlayerEntity player : mc.world.getPlayers()) {
            if (player.isSpectator() || player == mc.player || player.getHealth() <= 0 || Friends.get().isFriend(player))
                continue;
    ...
    private boolean selfNearCheck(Hole hole) {
        ...
        if (iSelfHole.get() && (HoleUtils.inHole(mc.player) || BlockUtils.collidable(pos))) return false;
    ...
    private boolean nearCheck(AbstractClientPlayerEntity player, Hole hole, double pDist) {
        ...
        if ((HoleUtils.inHole(player) || BlockUtils.collidable(pos)) && iHole.get()) return false;
