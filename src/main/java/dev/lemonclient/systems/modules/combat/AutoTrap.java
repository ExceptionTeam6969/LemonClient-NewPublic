                ...
                if (!toPlace.isEmpty()) {
                    ...
                    if (obsidian >= 0) {
                        ...
                        for (int i = 0; i < Math.min(obsidian, toPlace.size()); i++) {
                            ...
                            if (placeData.valid()) {
                                boolean rotated = !SettingUtils.shouldRotate(RotationType.BlockPlace) || Managers.ROTATION.start(placeData.pos().offset(placeData.dir()), 1, RotationType.BlockPlace, Objects.hash(name + "placing"));
                                ...
                                if (!switched) {
                                    if (hand == null) {
                                        ...
                                        switch (switchMode.get()) {
                                            ...
                                            case PickSilent -> InvUtils.pickSwitch(inventory.slot());
                                            case InvSwitch -> InvUtils.invSwitch(inventory.slot());
                                        }
                                    }
                                }
