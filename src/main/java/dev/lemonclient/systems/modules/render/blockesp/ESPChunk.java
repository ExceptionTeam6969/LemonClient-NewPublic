...
public class ESPChunk {
    ...
    public boolean shouldBeDeleted() {
        ...
        int chunkX = ChunkSectionPos.getSectionCoord(LemonClient.mc.player.getBlockPos().getX());
        int chunkZ = ChunkSectionPos.getSectionCoord(LemonClient.mc.player.getBlockPos().getZ());
    ...
    public static ESPChunk searchChunk(Chunk chunk, List<Block> blocks) {
        ...
        for (int x = chunk.getPos().getStartX(); x <= chunk.getPos().getEndX(); x++) {
            for (int z = chunk.getPos().getStartZ(); z <= chunk.getPos().getEndZ(); z++) {
                ...
                for (int y = LemonClient.mc.world.getBottomY(); y < height; y++) {
                    blockPos.set(x, y, z);
