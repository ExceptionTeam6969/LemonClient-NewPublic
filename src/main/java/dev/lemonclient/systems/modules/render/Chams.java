...
public class Chams extends Module {
    ...
    // Hand Color
    private final Setting<Boolean> enableHandColor = sgHand.add(new BoolSetting.Builder()
        .name("Enable")
        .description("Enable or disable custom rendering of hand items.")
        .defaultValue(false)
        .build()
    );
    private final Setting<SettingColor> handItemsColor = sgHand.add(new ColorSetting.Builder()
        .name("Hand Items Color")
        .description("Adjust the color of rendered hand items.")
        .defaultValue(new SettingColor(new java.awt.Color(0x9317DE5D, true)))
        .visible(enableHandColor::get)
        .build()
    );

    public static final Identifier BLANK = new Identifier(LemonClient.MOD_ID, "textures/blank.png");

    @EventHandler
    public void onRenderHands(HeldItemRendererEvent event) {
        if (enableHandColor.get()) {
            RenderSystem.setShaderColor(handItemsColor.get().r / 255f, handItemsColor.get().g / 255f, handItemsColor.get().b / 255f, handItemsColor.get().a / 255f);
        }
    }
