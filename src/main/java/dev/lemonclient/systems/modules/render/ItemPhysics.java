            ...
            for (BakedQuad _quad : model.getQuads(null, face, random)) {
                ...
                for (int i = 0; i < 4; i++) {
                    switch (_quad.getFace()) {
                        case DOWN -> minY = Math.min(minY, quad.lemonclient$getY(i));
                        case UP -> maxY = Math.max(maxY, quad.lemonclient$getY(i));
                        case NORTH -> minZ = Math.min(minZ, quad.lemonclient$getZ(i));
                        case SOUTH -> maxZ = Math.max(maxZ, quad.lemonclient$getZ(i));
                        case WEST -> minX = Math.min(minX, quad.lemonclient$getX(i));
                        case EAST -> maxX = Math.max(maxX, quad.lemonclient$getX(i));
                    }
                }
            }
        }
