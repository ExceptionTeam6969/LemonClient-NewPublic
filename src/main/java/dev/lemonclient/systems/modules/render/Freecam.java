...
import dev.lemonclient.events.client.MouseScrollEvent;
...
import dev.lemonclient.systems.modules.movement.GuiMove;
...
public class Freecam extends Module {
    ...
    private final SettingGroup sgMiningTrace = settings.createGroup("Mining Trace");
    ...
    private final Setting<Boolean> noMiningTrace = sgMiningTrace.add(new BoolSetting.Builder()
        .name("No Mining Trace")
        .description("Allows you to mine blocks through entities.")
        .defaultValue(true)
        .build()
    );
    private final Setting<Boolean> onlyWhenHoldingPickaxe = sgMiningTrace.add(new BoolSetting.Builder()
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace)) {
            ...
            if (mc.crosshairTarget instanceof EntityHitResult result) {
                crossHairPos = result.getEntity().getBlockPos();
                Managers.ROTATION.start(crossHairPos, 0, RotationType.BlockPlace, Objects.hash(name + "placing"));
            } else {
                ...
                if (!mc.world.getBlockState(crossHairPos).isAir()) {
                    Managers.ROTATION.start(Rotations.getYaw(crossHairPosition), Rotations.getPitch(crossHairPosition), 1, RotationType.BlockPlace, Objects.hash(name + "placing"));
                }
            }
        }
    ...
    public boolean canWork() {
        if (!isActive() && !noMiningTrace.get()) return false;
