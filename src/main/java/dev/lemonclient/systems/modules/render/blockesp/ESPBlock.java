...
public class ESPBlock {
    ...
    public ESPBlock getSideBlock(int side) {
        switch (side) {
            case FO:
                return blockEsp.getBlock(x, y, z + 1);
            case BA:
                return blockEsp.getBlock(x, y, z - 1);
            case LE:
                return blockEsp.getBlock(x - 1, y, z);
            case RI:
                return blockEsp.getBlock(x + 1, y, z);
            case TO:
                return blockEsp.getBlock(x, y + 1, z);
            case BO:
                return blockEsp.getBlock(x, y - 1, z);
        }
    ...
    public void update() {
        state = LemonClient.mc.world.getBlockState(blockPos.set(x, y, z));
        neighbours = 0;
    ...
    private boolean isNeighbour(Direction dir) {
        ...
        BlockState neighbourState = LemonClient.mc.world.getBlockState(blockPos);

        if (neighbourState.getBlock() != state.getBlock()) return false;

        VoxelShape cube = VoxelShapes.fullCube();
        VoxelShape shape = state.getOutlineShape(LemonClient.mc.world, blockPos);
        VoxelShape neighbourShape = neighbourState.getOutlineShape(LemonClient.mc.world, blockPos);
    ...
    private boolean isNeighbourDiagonal(double x, double y, double z) {
        ...
        return state.getBlock() == LemonClient.mc.world.getBlockState(blockPos).getBlock();
    }
