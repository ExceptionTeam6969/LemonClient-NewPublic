    ...
    private void onTick(TickEvent.Post event) {
        if ((waterMode.get() == Mode.Bob && mc.player.isTouchingWater()) || (lavaMode.get() == Mode.Bob && mc.player.isInLava())) {
            ...
            if (mc.player.isTouchingWater() && fluidHeight > swimHeight) {
                ((ILivingEntity) mc.player).swimUpwards(FluidTags.WATER);
            } else if (mc.player.isOnGround() && fluidHeight <= swimHeight && ((ILivingEntity) mc.player).getJumpCooldown() == 0) {
                mc.player.jump();
                ((ILivingEntity) mc.player).setJumpCooldown(10);
            } else {
                ((ILivingEntity) mc.player).swimUpwards(FluidTags.LAVA);
            }
        }
