 ...
 */
package dev.lemonclient.systems.modules.movement.elytrafly.modes;

import dev.lemonclient.events.packets.PacketEvent;
import dev.lemonclient.systems.modules.movement.elytrafly.ElytraFlightMode;
        ...
        if (checkConditions(mc.player)) {
            ...
            if (!rubberbanded) {
                if (prevFov != 0 && !elytraFly.sprint.get())
                    mc.options.getFovEffectScale().setValue(0.0); // This stops the FOV effects from constantly going on and off.
