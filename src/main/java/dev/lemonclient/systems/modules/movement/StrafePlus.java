...
public class StrafePlus extends Module {
    public StrafePlus() {
        super(Categories.Movement, "Strafe+", "Increase speed and control.");
    }
    ...
    private ElytraFly efly = null;
    private BurrowMovement burrowMovement = null;

    @Override
    public void onActivate() {
        timerClass = Modules.get().get(Timer.class);
        anchor = Modules.get().get(Anchor.class);
        longJump = Modules.get().get(LongJump.class);
        flight = Modules.get().get(Flight.class);
        efly = Modules.get().get(ElytraFly.class);
        burrowMovement = Modules.get().get(BurrowMovement.class);
    }
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (burrowMovement.isActive() && EntityInfo.isBurrowed(mc.player) || EntityInfo.isWebbed(mc.player)) return;

        if (Modules.get().isActive(HoleSnap.class)) return;

        if (EntityInfo.isWebbed(mc.player) && webbedPause.get() == WebbedPause.Always) return;
        if (EntityInfo.isWebbed(mc.player) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;

        if (mc.player.isOnGround()) {
            timerClass.setOverride(PlayerUtils.isMoving() ? (groundTimer.get() * TickRate.INSTANCE.getTPSMatch(TPSSync.get())) : Timer.OFF);
        } else {
            timerClass.setOverride(PlayerUtils.isMoving() ? (airTimer.get() * TickRate.INSTANCE.getTPSMatch(TPSSync.get())) : Timer.OFF);
        }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (BlockInfo.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always)
            return;
        if (BlockInfo.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir)
            return;
