...
import dev.lemonclient.utils.player.CraftUtils;
...
import static dev.lemonclient.utils.entity.EntityInfo.isInHole;
import static dev.lemonclient.utils.misc.Vec3dInfo.closestVec3d;
import static dev.lemonclient.utils.player.CraftUtils.getEmptySlots;
import static dev.lemonclient.utils.player.CraftUtils.isInventoryFull;
import static dev.lemonclient.utils.world.BlockInfo.getBlock;
import static dev.lemonclient.utils.world.BlockInfo.getSphere;

public class BedCrafter extends Module {
    public BedCrafter() {
        super(Categories.Player, "Bed Crafter", "Automatically craft beds.");
    }
    ...
    private final Setting<Boolean> automatic = sgAuto.add(new BoolSetting.Builder().name("automatic").description("Automatically place/search for and open crafting tables when you're out of beds.").defaultValue(true).build());
        ...
        for (BlockPos block : nearbyBlocks) {
            if (getBlock(block) == Blocks.AIR) {
                if (SettingUtils.shouldRotate(RotationType.BlockPlace))
                    Managers.ROTATION.start(block, priority, RotationType.BlockPlace, Objects.hash(name + "placing"));

                InvUtils.swap(craftTable.slot(), true);

                placeBlock(block, craftTable, true);
    ...
    private boolean canRefill(boolean checkSlots) {
        if (!autoWhileMoving.get() && PlayerUtils.isMoving()) return false;
