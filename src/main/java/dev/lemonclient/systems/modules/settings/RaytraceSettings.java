...
import dev.lemonclient.mixin.IRaycastContext;
...
public class RaytraceSettings extends Module {
    public RaytraceSettings() {
        super(Categories.Settings, "Raytrace", "Global raytrace settings for every lemon module.");
    }
    ...
    public boolean placeTrace(BlockPos pos) {
        ...
        switch (placeMode.get()) {
            case SinglePoint -> {
                ...
                result = DamageInfo.raycast(raycastContext);
    ...
    public boolean attackTrace(Box box) {
        ...
        switch (attackMode.get()) {
            case SinglePoint -> {
                ((dev.lemonclient.mixininterface.IRaycastContext) DamageInfo.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);

                return DamageInfo.raycast(DamageInfo.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case DoublePoint -> {
                ((dev.lemonclient.mixininterface.IRaycastContext) DamageInfo.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight1.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                if (DamageInfo.raycast(DamageInfo.raycastContext).getType() != HitResult.Type.BLOCK) {
                    return true;
                }

                ((dev.lemonclient.mixininterface.IRaycastContext) DamageInfo.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight2.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                return DamageInfo.raycast(DamageInfo.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case Exposure -> {
