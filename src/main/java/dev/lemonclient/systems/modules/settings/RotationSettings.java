...
import dev.lemonclient.managers.impl.RotationManager;
...
public class RotationSettings extends Module {
    public RotationSettings() {
        super(Categories.Settings, "Rotate", "Global rotation settings for every lemon module.");
    }
    ...
    public final Setting<Boolean> overrideRotations = sgGeneral.add(new BoolSetting.Builder()
        .name("Override Rotations")
        .description("Override all rotations")
        .defaultValue(false)
        .build()
    );
    ...
    public final Setting<Boolean> strafeFix = boolSetting(sgGeneral, "strafe-fix", false);
    public final Setting<Boolean> silentStrafe = boolSetting(sgGeneral, "silent-strafe", false, strafeFix::get);
    public final Setting<Integer> sprintTicks = intSetting(sgGeneral, "strafe-sprint-ticks", 1, 0, 200, strafeFix::get);
    ...
    public boolean raytraceCheck(Vec3d pos, double y, double p, Box box) {
        double range = pos.distanceTo(CombatInfo.getMiddle(box)) + 3;
        ...
        for (float i = 0; i < 1; i += 0.01) {
            if (box.contains(pos.x + (end.x - pos.x) * i, pos.y + (end.y - pos.y) * i, pos.z + (end.z - pos.z) * i))
                return true;
        }
        return false;
    }
