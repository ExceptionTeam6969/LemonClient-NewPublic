...
public class RangeSettings extends Module {
    public RangeSettings() {
        super(Categories.Settings, "Range", "Global range settings for every lemon module.");
    }

    private final SettingGroup sgPlace = settings.createGroup("Placing");
    private final SettingGroup sgAnchorPlace = settings.createGroup("Anchor Placing");
    private final SettingGroup sgCrystalPlace = settings.createGroup("Crystal Placing");
    ...
    public final Setting<Integer> radius = sgAnchorPlace.add(new IntSetting.Builder()
        .name("Radius")
        .description("Radius for respawn anchor placing.")
        .defaultValue(4)
        .min(1)
        .build()
    );
    public final Setting<Double> anchorPlaceRange = sgAnchorPlace.add(new DoubleSetting.Builder()
        .name("Anchor Place Range")
        .description("Range for respawn anchor placing.")
        .defaultValue(6)
        .range(0, 6)
        .sliderRange(0, 6)
        .build()
    );
    public final Setting<Double> anchorPlaceRangeWalls = sgAnchorPlace.add(new DoubleSetting.Builder()
        .name("Anchor Place Walls Range")
        .description("Range for placing behind respawn anchor.")
        .defaultValue(6)
        .range(0, 6)
        .sliderRange(0, 6)
        .build()
    ...
    //  Crystal Place Ranges
    public final Setting<Double> crystalPlaceRange = sgCrystalPlace.add(new DoubleSetting.Builder()
        .name("Crystal Place Range")
        .description("Range for end-crystal placing.")
        .defaultValue(6)
        .range(0, 6)
        .sliderRange(0, 6)
        .build()
    );
    public final Setting<Double> crystalPlaceRangeWalls = sgCrystalPlace.add(new DoubleSetting.Builder()
        .name("Crystal Place Walls Range")
        .description("Range for placing behind end-crystal.")
        .defaultValue(6)
        .range(0, 6)
        .sliderRange(0, 6)
        .build()
    ...
    public double placeRangeTo(BlockPos pos, Vec3d from) {
        ...
        switch (placeRangeMode.get()) {
            ...
            case Vanilla -> {
                return getRange(from, CombatInfo.getClosest(mc.player.getEyePos(), feet, 1, 1));
            }
            case CustomBox -> {
                return getRange(from, CombatInfo.getClosest(mc.player.getEyePos(), feet, blockWidth.get(), blockHeight.get()));
            }
        }
        return -1;
    }

    // Anchor Place Range Checks
    public boolean inAnchorPlaceRange(BlockPos pos) {
        if (mc.player == null) {
            return false;
        }

        double dist = placeRangeTo(pos, null);
        return dist >= 0 && dist <= (SettingUtils.placeTrace(pos) ? anchorPlaceRange.get() : anchorPlaceRangeWalls.get());
    }

    // Crystal Place Range Checks
    public boolean inCrystalPlaceRange(BlockPos pos, Vec3d from) {
        if (mc.player == null) {
            return false;
        }

        double dist = placeRangeTo(pos, from);
        return dist >= 0 && dist <= (SettingUtils.placeTrace(pos) ? crystalPlaceRange.get() : crystalPlaceRangeWalls.get());
    }
    ...
    public double attackRangeTo(Box bb, Vec3d feet, Vec3d from, boolean countReduce) {
        ...
        if (from == null) {
            ...
            switch (attackRangeFrom.get()) {
                case Middle -> from = from.add(0, mc.player.getEyeHeight(mc.player.getPose()) / 2, 0);
                case Eyes -> from = from.add(0, mc.player.getEyeHeight(mc.player.getPose()), 0);
            }
        }
        ...
        double dist = switch (attackRangeMode.get()) {
            ...
            case Vanilla ->
                getRange(from, CombatInfo.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX), Math.abs(bb.minY - bb.maxY)));

            case Middle ->
                getRange(from, new Vec3d((bb.minX + bb.maxX) / 2, (bb.minY + bb.maxY) / 2, (bb.minZ + bb.maxZ) / 2));

            case CustomBox ->
                getRange(from, CombatInfo.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX) * closestAttackWidth.get(), Math.abs(bb.minY - bb.maxY) * closestAttackHeight.get()));
    ...
    public double miningRangeTo(BlockPos pos, Vec3d from) {
        ...
        switch (miningRangeMode.get()) {
            ...
            case CustomBox -> {
                return getRange(from, CombatInfo.getClosest(mc.player.getEyePos(), feet, closestMiningWidth.get(), closestMiningHeight.get()));
            }
        }
        return -1;
    }
