...
import dev.lemonclient.mixin.ICloseHandledScreenC2SPacket;
import dev.lemonclient.mixin.IHandledScreen;
...
public class InvTweaks extends Module {
    public InvTweaks() {
        super(Categories.Misc, "Inv Tweaks", "Various inventory related utilities.");
    }
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((IHandledScreen) screen).getFocusedSlot();
    ...
    public void steal(ScreenHandler handler) {
        Executor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        Executor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
