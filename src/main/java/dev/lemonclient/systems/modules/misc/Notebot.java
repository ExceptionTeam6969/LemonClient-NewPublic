...
import dev.lemonclient.utils.notebot.NotebotUtils;
import dev.lemonclient.utils.notebot.decoder.SongDecoder;
import dev.lemonclient.utils.notebot.decoder.SongDecoders;
import dev.lemonclient.utils.notebot.instrumentdetect.InstrumentDetectMode;
import dev.lemonclient.utils.notebot.song.Note;
import dev.lemonclient.utils.notebot.song.Song;
import dev.lemonclient.utils.player.RaycastUtils;
...
public class Notebot extends Module {
    ...
    public final Setting<Boolean> vanillaRotation = boolSetting(sgGeneral, "vanilla-rotation", false, autoRotate::get);
    ...
    public final Setting<Boolean> directionCheck = boolSetting(sgGeneral, "direction-check", false);
    ...
    public void playRandomSong() {
        File[] files = LemonClient.FOLDER.toPath().resolve("notebot").toFile().listFiles();
        ...
        loadingSongFuture.whenComplete((song, ex) -> {
            if (ex == null) {
                ...
                if (ex instanceof CancellationException) {
                    ...
                    LemonClient.LOG.error("An error occurred while loading song '" + FilenameUtils.getBaseName(file.getName()) + "'", ex);
                    onSongEnd();
                }
            }
        });
    ...
    private void tuneBlocks() {
        ...
        while (iterator.hasNext()) {
            ...
            this.tuneNoteblockWithPackets(yaw, pitch, pos);
    ...
    private void tuneNoteblockWithPackets(float yaw, float pitch, BlockPos pos) {
        // We don't need to raycast here. Server handles this packet fine -> 666
        BlockHitResult blockHitResult = new BlockHitResult(Vec3d.ofCenter(pos), Direction.DOWN, pos, false);
        if (directionCheck.get()) {
            HitResult hitResult = RaycastUtils.raycast(yaw, pitch, mc.interactionManager.getReachDistance(), true);
            if (hitResult != null && !hitResult.getType().equals(HitResult.Type.ENTITY)) {
                blockHitResult = RaycastUtils.fixHitResult((BlockHitResult) hitResult, pos);
            }

            yaw = RaycastUtils.getYawByDirection(pos, blockHitResult.getSide());
            pitch = RaycastUtils.getPitchByDirection(pos, blockHitResult.getSide());
        }

        BlockHitResult finalBlockHitResult = blockHitResult;
        Runnable task = () -> {
            interactBlock(Hand.MAIN_HAND, finalBlockHitResult.getPos(), finalBlockHitResult.getSide(), finalBlockHitResult.getBlockPos());
            //mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND,finalBlockHitResult, 0));
            anyNoteblockTuned = true;
        };
        if (autoRotate.get()) {
            Rotations.rotate(yaw, pitch, 100, task);
            return;
        }
        ...
        if (!notes.isEmpty()) {
            ...
            if (autoRotate.get()) {
                ...
                if (firstNote.isPresent()) {
                    ...
                    if (firstPos != null) {
                        if (vanillaRotation.get()) {
                            float yaw = (float) Rotations.getYaw(firstPos);
                            float pitch = (float) Rotations.getPitch(firstPos);
                            mc.player.setYaw(yaw);
                            mc.player.setPitch(pitch);
                        } else
                            Managers.ROTATION.start(firstPos, priority, RotationType.Interact, Objects.hash(name + "play"));
                    }
                }
            }
    ...
    private void playRotate(BlockPos pos) {
        ...
        try {
            float yaw = Managers.ROTATION.currentDir[0];
            float pitch = Managers.ROTATION.currentDir[1];
            BlockHitResult blockHitResult = new BlockHitResult(Vec3d.ofCenter(pos), Direction.DOWN, pos, false);
            if (directionCheck.get()) {
                HitResult hitResult = RaycastUtils.raycast(yaw, pitch, mc.interactionManager.getReachDistance(), true);
                if (hitResult != null && !hitResult.getType().equals(HitResult.Type.ENTITY)) {
            ...
            mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, pos, blockHitResult.getSide(), 0));
