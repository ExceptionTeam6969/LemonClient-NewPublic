...
public class Mesh {
    ...
    private final Attrib[] attributes;

    private int attribPosition;

    public Attrib getCurrentAttrib() {
        return attributes[attribPosition];
    }
    ...
    public void begin() {
        ...
        if (rendering3D) {
            Vec3d camera = LemonClient.mc.gameRenderer.getCamera().getPos();
    ...
    public Mesh uv(double u, double v) {
        long p = verticesPointer;
    ...
    public void beginRender(MatrixStack matrices, float lineWidth) {
        GL.saveState();
        ...
        if (rendering3D) {
            ...
            if (Utils.canUpdate()) {
                Vec3d cameraPos = LemonClient.mc.gameRenderer.getCamera().getPos();
    ...
    public void render(MatrixStack matrices, float lineWidth) {
        ...
        if (indicesCount > 0) {
            ...
            if (!wasBeganRendering) beginRender(matrices, lineWidth);

            // Render
            beforeRender();

            Shader.BOUND.setDefaults();
            if (SharedShaderMesh.BOUND != null) {
                SharedShaderMesh.BOUND.setUniforms();
            }
