...
import dev.lemonclient.gui.screens.settings.*;
...
public class DefaultSettingsWidgetFactory extends SettingsWidgetFactory {
    ...
    private void fontW(WTable table, FontFaceSetting setting) {
        WHorizontalList list = table.add(theme.horizontalList()).expandX().widget();
        dev.lemonclient.gui.widgets.WLabel label = list.add(theme.label(setting.get().info.family())).widget();
    ...
    private static class WSelectedCountLabel extends WLabel {
