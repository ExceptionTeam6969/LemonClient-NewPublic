 ...
 *----------------------------------------------------------------------
 */
package dev.lemonclient.music.player.decoder.mp3;
...
final class SynthesisFilter {
    ...
    private static final Identifier SFD_SER = new Identifier(LemonClient.MOD_ID, "decoder/sfd.ser");
    ...
    static private float[] load_d() {
        ...
        Object o = null;
        try {
            o = JavaLayerUtils.deserializeArray(MinecraftClient.getInstance().getResourceManager().getResource(SFD_SER).orElseThrow().getInputStream(), elemType, 512);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return (float[]) o;
     ...
     */
    static private float[][] splitArray(final float[] array) {
